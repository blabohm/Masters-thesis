...
networkCombinator(file_list, output_dir = "Z:/Berlin")

networkCombinator <- function(file_list, out = "file", file_name = "edges.gpkg",
                              output_dir = NULL)
{
  # load packages
  require(dplyr, quietly = TRUE)
  ...
  output_dir <- outputChecker(directory = output_dir, file_name = file_name)
  # set up progress bar
  ...
  for (file in file_list) {
    ...
    # Read input in file_list
    tmp <- file %>%
      st_read(quiet = TRUE) %>%
      filter(!st_is_empty(.)) %>%
      as_sfnetwork()
    # execute a network join
    if (!is.null(outSF)) {
      outSF <- tmp %>%
...
combinator <- function(file_list, out = "file", file_name = "nodes.gpkg", output_dir = NULL)
{
  # load packages
  require(dplyr, quietly = TRUE)
  require(sf, quietly = TRUE)
...
# setting_name: Setting description - DEFAULT values
################################################################################
outputChecker <- function(directory, file_name, output_type = "file", verbose = FALSE)
{
  if (!verbose) {
    # check directory
    ...
    if (file.exists(tmpOut)) {
      ...
      if (b == "y") unlink(tmpOut) else stop("Specify different file name.")
    }
    tmpOut
  }
}
